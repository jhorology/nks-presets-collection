...
<UVI4>
  ...
  <NeededFS Source="/Library/Application Support/UVISoundBanks/Digital Synsations.ufs"/>
</UVI4>
'''
  ufs: '$Digital Synsations.ufs'
  # Ableton Live 10.1.2 Instrument Rack
  abletonRackTemplate: 'src/Digital Synsations/templates/Digital Synsations.adg.tpl'
  # Bitwig Studio 3.0.3 preset file
  bwpresetTemplate: 'src/Digital Synsations/templates/Digital Synsations.bwpreset'
  # bank
  models:
    DS1:   'KORG M1'
    DS77:  'YAMAHA SY77'
...
gulp.task "#{$.prefix}-generate-meta", ->
  ...
  gulp.src ["#{presets}/**/*.M5p"], read: on
    .pipe data (file) ->
      basename = path.basename file.path, '.M5p'
      folderChain = file.relative.split '/'
      bankName = $.models[folderChain[0]]
      unless bankName
        throw new Error "undefined model. id: #{folderChain[0]}"
...
gulp.task "#{$.prefix}-dist-presets", ->
  ...
  gulp.src ["src/#{$.dir}/presets/**/*.M5p"], read: on
    .pipe data (uvip) ->
