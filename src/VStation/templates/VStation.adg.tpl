			...
			<InstrumentBranchPreset>
				...
				<DevicePresets>
					<VstPreset>
						...
						<ProgramCount Value="400" />
						<ParameterCount Value="179" />
						<ProgramNumber Value="0" />
						<Buffer><% _.forEach(bufferLines, function(line) { %>
							<%=line%><% }); %>
						</Buffer>
						<Name Value="" />
						<PluginVersion Value="2300" />
						<UniqueId Value="1316377392" />
						<ByteOrder Value="2" />
						<PresetRef />
					</VstPreset>
				</DevicePresets>
				...
				<SourceContext>
					<BranchSourceContext>
						...
						<PresetRef />
						<BranchDeviceId Value="device:vst:instr:1316377392?n=VStation" />
					</BranchSourceContext>
				</SourceContext>
				<ZoneSettings>
					<KeyRange>
