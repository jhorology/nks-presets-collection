	...
	<GroupDevicePreset>
		...
		<Device>
			<InstrumentGroupDevice Id="0">
				...
				<AreDevicesVisible Value="true" /><% _.forEach(macros.slice(0, 8), function(macro, index) { %>
				<MacroControls.<%=index%>>
					<LomId Value="0" />
				...
				</MacroControls.<%=index%>><% }); _.forEach(macros.slice(0, 8), function(macro, index) { %>
		...
		<BranchPresets>
			<InstrumentBranchPreset Id="0">
				...
				<DevicePresets>
					<VstPreset Id="0">
						<OverwriteProtectionNumber Value="2561" />
						<ParameterSettings><% for(var i = 0; i < 8; i++) { %>
							<PluginParameterSettings Id="<%=i%>">
								<Index Value="<%=i%>" />
								<VisualIndex Value="<%=i%>" />
								<ParameterId Value="<%=i%>" />
								<Type Value="PluginFloatParameter" />
								<MacroControlIndex Value="<%=i%>" />
								<MidiControllerRange>
								...
								<LomId Value="0" />
							</PluginParameterSettings><% } for(var i = 8; i < 16; i++) {%>
							<PluginParameterSettings Id="<%=i%>">
								<Index Value="<%=i%>" />
								<VisualIndex Value="<%=i%>" />
								<ParameterId Value="<%=i%>" />
